...
- `claude-usage-tray.ps1` is a standalone PowerShell script — no npm, no build step — using only `System.Windows.Forms`/`System.Drawing` (ships with Windows). It creates two `NotifyIcon`s (session, week), each rendered by `New-GaugeIcon` as a small circular gauge (dark card, muted track ring, tier-colored arc proportional to the percentage, number in cream at the center) onto a generated bitmap, mirroring the plugin's `buildGaugeSvg` look. Colors come from the same 70%/90% tiers as the plugin (`Get-TierColor`, reusing the plugin's `CLAUDE`-palette hex values).
- A solid tier-colored fill (dropping the ring so the digits could be bigger) was tried and reverted — the user found it "horrível" on sight. The ring stays even though the icon is small; don't swap back to a solid fill without being asked.
